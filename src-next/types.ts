// Public type surface for the instance-based QZ client.
//
// These replace the large JSDoc `@param options.*` blocks in the legacy
// `src/api/*.js` files with compiler-checked interfaces. Everything a consumer
// needs to type a `createQz(...)` call lives here.
...
export interface PrintOptions {
    ...
    copies?: number;
    density?: number | number[] | { cross?: number; feed?: number } | 'best' | 'draft';
    ...
    forceRaw?: boolean;
    encoding?: string | { from?: string; to?: string } | null;
    endOfDoc?: string | null;
    perSpool?: number;
    retainTemp?: boolean;
    spool?: SpoolOptions | null;

    // Version-shim outputs (set by the compatibility layer, not by users).
    altPrinting?: boolean;
}

/** Printer target: a name, or an object specifying host/file routing. */
export type PrinterInput = string | { name?: string; host?: string; port?: string; file?: string };
...
export interface LnaShim {
    detectLna<T>(
        ...
        opts: { isWebSocket: boolean; defaultAddressSpace: string }
    ): Promise<T>;
